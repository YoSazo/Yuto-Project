...
function getSupabaseClient() {
  if (!SUPABASE_URL || !SUPABASE_SERVICE_ROLE_KEY) {
    ...
    throw new Error(
      `Missing Supabase env in share-referral: ${missing.join(", ")}. Use service role key (not anon) for server-side.`,
    );
  }
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const canonicalUrl = `${origin}/r/${encodeURIComponent(username)}`;
  const ogImageUrl = `${origin}/og/referral/${encodeURIComponent(username)}.png`;
...
<html lang="en">
  <head>
    ...
    <meta http-equiv="refresh" content="0; url=/referral/${encodeURIComponent(username)}" />
  </head>
  <body></body>
</html>`;
