...
export function DmChargeModal({
  ...
  return (
    <div className="fixed inset-0 z-[55] flex items-end md:items-center justify-center fade-in bg-black/60 backdrop-blur-sm">
      ...
      <div className="relative z-10 bg-white rounded-t-3xl md:rounded-3xl w-full max-w-md p-6 modal-slide-up shadow-xl">
        <div className="flex items-center justify-between mb-4">
          <div>
            <p className="font-extrabold text-black text-lg">Charge</p>
            <p className="text-xs text-gray-400 font-semibold">Buyer pays from Yuto Balance</p>
          </div>
          <button type="button" onClick={onClose} className="text-2xl text-gray-400 hover:text-black bg-transparent border-none">
            <X size={22} />
          </button>
        </div>
        ...
        <div className="space-y-4">
          <div>
            ...
            <input
              ...
              placeholder="6000"
              className="w-full text-3xl font-black tracking-tight outline-none border border-gray-200 rounded-2xl px-4 py-3"
            />
          </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              <button
                ...
                className={`rounded-2xl py-3 px-3 text-sm font-bold border transition-colors ${
                  releaseMode === "held" ? "bg-black text-white border-black" : "bg-gray-50 text-gray-600 border-gray-200"
                }`}
              >
                After handoff
              ...
              <button
                ...
                className={`rounded-2xl py-3 px-3 text-sm font-bold border transition-colors ${
                  releaseMode === "trust" ? "bg-emerald-600 text-white border-emerald-600" : "bg-gray-50 text-gray-600 border-gray-200"
                }`}
              >
                Trust / delivery
          ...
          {listings.length > 0 && (
            <div>
              ...
              <select
                ...
                className="w-full h-12 rounded-2xl border border-gray-200 px-3 font-semibold text-sm"
              >
          ...
          <div>
            ...
            <input
              ...
              maxLength={200}
              className="w-full h-12 rounded-2xl border border-gray-200 px-4 text-sm font-semibold outline-none focus:border-black"
            />
          </div>
          ...
          <button
            ...
            disabled={busy}
            onClick={() => void submit()}
            className={`w-full py-4 rounded-full font-bold text-lg transition-colors ${busy ? "bg-gray-100 text-gray-400" : "bg-black text-white active:scale-[0.99]"}`}
          >
