    ...
    <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center fade-in bg-black/60 backdrop-blur-sm">
      ...
      <div className="relative z-10 bg-white rounded-t-3xl md:rounded-3xl w-full max-w-md p-5 modal-slide-up">
        <div className="flex items-center justify-between mb-4">
          <p className="font-extrabold text-black text-lg">Tag people</p>
              ...
              return (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="font-bold text-black truncate">{p.display_name || p.username}</p>
                    <p className="text-sm text-gray-400 truncate">@{p.username}</p>
                  </div>
                  <div
                    className={`w-5 h-5 rounded-full border-2 ${picked ? "bg-black border-black" : "border-gray-300"}`}
                  />
                </button>
              );
            })}
        ...
        <div className="mt-4 flex items-center gap-2">
          <button
            ...
            className="flex-1 h-12 rounded-full bg-gray-100 text-black font-extrabold hover:bg-gray-200"
          >
            Clear
          </button>
          <button
            type="button"
            onClick={onClose}
            className="flex-1 h-12 rounded-full bg-black text-white font-extrabold"
          >
