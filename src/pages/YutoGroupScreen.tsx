...
// ── Pay Your Share Modal (direct M-PESA to group invoice) & balance top-up ────
function PayNowModal({
  ...
  amount: number;
  /** Required when paying into a group invoice; omit for wallet top-up */
  groupId?: string;
  userId: string;
  ...
  const handlePay = async () => {
    ...
    if (!isTopUp && !groupId) {
      setError("Missing group. Close and try again.");
      return;
    }
    setStep("sending");
    setError("");
      ...
      const res = await fetch("/api/charge", {
        ...
        body: JSON.stringify(
          isTopUp
            ...
            : { phone_number: phone, amount, group_id: groupId, user_id: userId },
        ),
      });
  ...
  const title = isTopUp ? "Top up Yuto Balance" : "Pay with M-PESA";
  const cta = isTopUp ? "Top up" : "Pay";
    ...
    step === "input" || step === "error" ? (
      <>
        <div className="flex justify-between items-center mb-5">
          ...
          <button onClick={onClose} type="button" className="text-2xl text-gray-400 hover:text-black bg-transparent border-none cursor-pointer">
            ✕
          </button>
        </div>
        {!embedded && (
        ...
        <button
          ...
          className={`w-full h-12 rounded-full font-bold text-base transition-colors ${phone.length >= 12 ? "bg-black text-white hover:bg-gray-800" : "bg-gray-200 text-gray-400 cursor-not-allowed"}`}
        >
          {cta} KSH {amount.toLocaleString()}
        </button>
        {step === "error" && onRefreshStatus && (
          <button type="button" onClick={() => void onRefreshStatus()} className="mt-3 w-full text-sm text-gray-500 underline hover:text-black text-center">
      ...
      <div className="py-12 text-center">
        ...
        {onRefreshStatus && (
          <button type="button" onClick={() => void onRefreshStatus()} className="mt-4 text-sm text-gray-500 underline hover:text-black">
            I already paid — refresh and pay share
          </button>
        )}
      </div>
    );
...
export default function YutoGroupScreen() {
  ...
  // Auto-close pay modal when real-time confirms payment
  useEffect(() => {
